...
fn wrap_decrement(start: u16, decrement: u16, maximum: u16) u16 {
    return (start + maximum - decrement) % maximum;
}

fn wrap_increment(start: u16, increment: u16, maximum: u16) u16 {
    return (start + increment) % maximum;
}

fn pattern_length_at(soup: anytype, start: u16, comptime limit: u16) ?u16 {
    var length: u16 = 0;

    while (length < limit) {
        if (!is_nop(soup.read(wrap_increment(start, length, soup.len)))) {
            break;
        }
...
fn search_forward_match(soup: anytype, start: u16, pattern_length: u16, comptime limit: u16) ?u16 {
...
pub fn search_backward(soup: anytype, start: u16, comptime limit: u16) ?u16 {
    const pattern_length = pattern_length_at(soup, start, limit) orelse return null;
...
fn search_backward_match(soup: anytype, start: u16, pattern_length: u16, comptime limit: u16) ?u16 {
...
pub fn search_bidirectional(soup: anytype, start: u16, comptime limit: u16) ?u16 {
    const pattern_length = pattern_length_at(soup, start, limit) orelse return null;
